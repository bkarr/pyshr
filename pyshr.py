from __about__ import (
    ...
    __uri__, __version__, __libshr_version__
)
...
class SharedQueue:
    ...
    def __to_vector(self, items, vector, vcnt, buffers):
        for i in xrange(vcnt):
            ...
            if d_type == int or (d_type == SHType.INTEGER_T and type(data) == int):
                vector[i].type = lib.SH_INTEGER_T
                vector[i].len = 4
                buffers.append(ffi.new("int *", data))
                vector[i].base = buffers[i]
            elif d_type == long or (d_type == SHType.INTEGER_T and type(data) == long):
                vector[i].type = lib.SH_INTEGER_T
                vector[i].len = 8
